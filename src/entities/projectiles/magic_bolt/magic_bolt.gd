...
func _hit(node: Node) -> void:
	...
	if node and (node.is_in_group("enemies") or node.is_in_group("destructibles")) and node.has_method("take_damage"):
		...
		node.take_damage(dealt, false, source_weapon.damage_type if source_weapon != null else DamageType.Type.ARCANE, false, source_weapon.get_ailment_effect_multiplier() if source_weapon != null else 1.0)
		...
		if node.has_method("apply_knockback"):
			node.apply_knockback(global_position, source_weapon.get_knockback(120.0) if source_weapon != null else 120.0)

...
- **Knight** — primary: *Knight Blade* (3-hit combo: two **forward-facing 100° arc slashes** instead of a triangle, then a heavy forward stab at 1.5×; slashes emanate from the player as a wide pie-slice sector, ~3× longer reach, buffed damage); secondary: *Tower Shield* (appears only while RMB held, has its own HP bar, HP persists between raises and only repairs after a full break + recharge; **shares the player's invincibility frames** — takes no HP damage while the player is i-framed; blocks enemy projectiles + shoves enemies back, and enemies touching it drain its HP; own HP/armor *lower* than player's, thorns *equal*; buffed HP, longer recharge after break)
...
- **Mage** — primary: *Arcane Bolts* (repurposed automatic Magic Missiles, now manual homing; bolts fly straight toward their target from frame 1, home smoothly without orbiting, and re-acquire a target if the original dies); secondary: *Mana Overload* (buffs to **halve all cooldowns** for 4s; the ability's own recharge only starts **after** the buff ends, unaffected by the buff; shows a pulsing purple/magenta aura indicator while active)
